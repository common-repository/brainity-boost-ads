...
class FacebookProduct
{
        ...
        $wpid, $parent_product = null)
    {
        ...
        if ($parent_product) {
            ...
            $this->main_description = FacebookUtils::clean_string(
            ...
            foreach ($image_ids as $image_id) {
                ...
                if (!empty($image_url)) {
                    array_push($gallery_urls,
                        FacebookUtils::make_url($image_url));
                }
            }
    ...
    public function get_all_image_urls()
    {
        ...
        if ($image_url) {
            $image_url = FacebookUtils::make_url($image_url);
        ...
        if ($parent_image_id) {
            ...
            $image_url2 = FacebookUtils::make_url($image_url2);
    ...
    {
        if (FacebookUtils::is_variation_type($this->woo_product->get_type())) {
    ...
    public function set_description($description)
    {
        $description = stripslashes(
            FacebookUtils::clean_string($description));
    ...
    public function set_product_image($image)
    {
        if ($image !== null && strlen($image) !== 0) {
            $image = FacebookUtils::clean_string($image);
            $image = FacebookUtils::make_url($image);
            update_post_meta(
                $this->id,
    ...
    public function get_fb_description()
    {
        ...
        if (FacebookUtils::is_variation_type($this->woo_product->get_type())) {
            $description = FacebookUtils::clean_string(
        ...
        $post_content = FacebookUtils::clean_string(
            $post->post_content);
        $post_excerpt = FacebookUtils::clean_string(
            $post->post_excerpt);
        $post_title = FacebookUtils::clean_string(
            $post->post_title);
    ...
    public function is_hidden()
    {
        ...
        $wpid = $this->id;
        if (FacebookUtils::is_variation_type($this->get_type())) {
        ...
        $prepare_for_product_feed = false)
    {
        if (!$retailer_id) {
            $retailer_id =
                FacebookUtils::get_fb_retailer_id($this);
        }
        ...
        if (!$product_type || !isset(self::$use_checkout_url[$product_type])) {
            ...
            $checkout_url = FacebookUtils::make_url(
        ...
        $categories =
            FacebookUtils::get_product_categories($id);
        $brand = get_the_term_list($id, 'product_brand', '', ', ');
        $brand = is_wp_error($brand) || !$brand
            ? FacebookUtils::get_store_name()
            : FacebookUtils::clean_string($brand);

        $product_data = array(
            'name' => FacebookUtils::clean_string(
                $this->get_title()),
        ...
        FacebookUtils::is_all_caps($product_data['description'])
        ) {
    ...
    {
        if (!FacebookUtils::is_variation_type(
            $this->get_type())) {
            return;
        }
        ...
        foreach ($variant_names as $orig_name) {
            ...
            $new_name = str_replace(
                'custom_data:',
                '',
                FacebookUtils::sanitize_variant_name($orig_name));
            ...
            if (isset($options)) {
                ...
                if ($new_name === FacebookUtils::FB_VARIANT_GENDER) {
                    // If we can't validate the gender, this will be null.
                    $product_data[$new_name] =
                        FacebookUtils::validateGender($option_values[0]);
                }

                switch ($new_name) {
                    case FacebookUtils::FB_VARIANT_SIZE:
                    case FacebookUtils::FB_VARIANT_COLOR:
                    case FacebookUtils::FB_VARIANT_PATTERN:
                        ...
                        break;
                    case FacebookUtils::FB_VARIANT_GENDER:
    ...
    {
        if (!FacebookUtils::is_variable_type(
            $this->get_type())) {
            FacebookUtils::fblog(
...
// For API approach, product_field need to start with 'custom_data:'
            // Clean up variant name (e.g. pa_color should be color)
            $name = FacebookUtils::sanitize_variant_name($name);
